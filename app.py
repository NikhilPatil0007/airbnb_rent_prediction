...
# MySQL configuration
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:nsp262003@localhost/airbnb'
...
def preprocess_data(df):
    ...
    # One more time, fill any missing values with median just in case
    ...
    df['neighbourhood_cleansed'] = df['neighbour_cleansed'].fillna(df['neighbourhood_cleansed'].mode()[0])

    # One-Hot encode the "Neighbourhood_ cleansed" column
    encoder = OneHotEncoder(sparse_output=False)
    neighbourhood_encoded = encoder.fit_transform(df[['neighbourhood_cleansed']])

    #Create a DataFrame for the one-hot encoded neighbourhoods
    neighbourhood_encoded_df = pd.DataFrame(neighbourhood_encoded, columns=encoder.get_feature_names_out(['neighbourhood_cleansed']))

    # Concatenate the encoded neighbourhood with original dataframe
...
def reload_data():
    ...
    try:
        ...
        decomprressed_file = gzip.GzipFile(fileobj=compressed_file)

        # Step 2: Load data into Pandas
        listings = pd.read_csv(decomprressed_file)

        # Step 3 : Clear the database
        db.session.query(Listing).delete()

        # Step 4: Process data and insert it into database
        ...
        db.session.commit()
        print(f"Inserted {len(listings)} rows into the database.")
        ...
        print("Data reloaded succesfully")
...
def predict():
    ...
    # Check if the model and encoder are initalized
